      ...
      <Link href={`/pages/movies/${movie.id}`}>
        <div className="movie-card flex items-end group">
          ...
          <Image
            ...
            width={300}
            height={450}
            className="border-2 border-transparent group-hover:border-gray-700 transition-all duration-500 max-w-[350px] max-h-[400px]"
          />
        </div>
      </Link>
    );
      ...
      <Link href={`/pages/movies/${movie.id}`}>
        <div className="relative group overflow-hidden rounded-lg">
          <Image
            ...
            priority={false}
            loading="lazy"
            className="group-hover:scale-110 transition-all duration-500 ease-in-out max-w-[250px] max-h-[300px] h-auto"
          />
